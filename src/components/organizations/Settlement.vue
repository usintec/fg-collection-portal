...
onMounted(async () => {
  try {
    const res = await PaymentService.getSettlementSettings()
...
const save = async (providerIndex: number, configIndex: number) => {
  ...
  try {
    if (state.value[providerIndex].settingId) {
      await PaymentService.updateSettlementSetting(state.value[providerIndex])
...
const addProvider = async (v: string) => {
  try {
    ...
    const res = await PaymentService.createSettlementSetting({
      provider: v,
      configurationEntries: []
    })
  ...
  try {
    ...
    if (deleteTask.value == DeleteTask.PROVIDER) {
      if (providerToDelete.value) {
        ...
        await PaymentService.deleteSettlementSetting(providerToDelete.value?.providerId)
...
// const removeConfigurationSetting = (providerIndex: number, settingIndex: number) =>
//   state.value[providerIndex].configurationEntries.splice(settingIndex, 1)
  ...
  if (configItemToDelete.value)
    try {
      if (state.value[configItemToDelete.value?.providerIndex].settingId) {
        ...
        await PaymentService.updateSettlementSetting({
...
<template>
  <div>
    <div v-if="!loadingSettlement">
      <div class="px-4 flex justify-between items-center">
        <div>
          <h3 class="text-sm font-medium">Settlement/Reconciliation configuration</h3>
          <p class="text-[#8492B6] text-xs font-normal pt-5">
            Configure your settlement and supply values for keys and setup
          </p>
        </div>
        <div class="w-[13%]">
          <ButtonInput
      ...
      <div class="my-7 w-full" v-for="(item, index) in state" :key="index">
          ...
          <div class="flex flex-col w-full">
            <div class="flex justify-between items-center my-3">
              ...
              <div class="w-1/2 xl:w-2/5 2xl:w-1/3 flex gap-5">
                ...
                <DeletePopup
                  @delete="() => removeAt()"
                  title="Delete Payment Settlement Configuration"
                  :show="confirmDelete"
