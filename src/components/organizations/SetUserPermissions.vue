...
import type { IPermission, IAdminUser, ICreateAdmin } from '@/services/types'
...
const isPermissionAvailable = (code: any) => {
  ...
  const permitValues = Object.values(props.permits).flatMap((group) => group)
...
const submit = async () => {
  ...
  const updatedUserInput = {
    ...props.userInput,
    permissions: Object.keys(isChecked.value).filter((code) => isChecked.value[code])
  }

  const adminDatas: ICreateAdmin = {
    organizationId: props.org ?? '',
    adminUserModel: updatedUserInput
  }
