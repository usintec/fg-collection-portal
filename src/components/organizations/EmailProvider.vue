...
const loadingEmailSettings = ref(true)
...
const save = async (providerIndex: number, configIndex: number) => {
  ...
  try {
    if (state.value[providerIndex].settingId) {
      await NotificationService.updateEmailSetting(state.value[providerIndex])
...
const addProvider = async (v: string) => {
  try {
    ...
    const res = await NotificationService.createEmailSetting({
      provider: v,
      configurationEntries: []
    })
  ...
  try {
    ...
    if (deleteTask.value == DeleteTask.PROVIDER) {
      if (providerToDelete.value) {
        ...
        await NotificationService.deleteEmailSetting(providerToDelete.value?.providerId)
  ...
  if (configItemToDelete.value)
    try {
      if (state.value[configItemToDelete.value?.providerIndex].settingId) {
        ...
        await NotificationService.updateEmailSetting({
...
<template>
  <div>
    <div v-if="!loadingEmailSettings">
      <div class="px-4 flex justify-between items-center">
        <div>
          <h3 class="text-sm font-medium">Email settings configuration</h3>
        ...
        <div class="mx-3 my-5 rounded-lg border py-2 px-4">
          <div class="flex flex-col w-full">
            <div class="flex justify-between items-center my-3">
              <div>
                <span class="bg-[#F3FFF9] p-2 rounded-md px-4 text-md text-[#1E754C] me-3"
            ...
            >
              ...
              <div class="w-24">
                <CheckPermission :permission="Permissions.UpdateNotificationPreference">
                  <ButtonInput
      ...
      <div class="flex justify-end mb-5 mx-3">
        <div class="w-[13%]" v-if="state.length > 0">
          <CheckPermission :permission="Permissions.CreateNotificationPreference">
            <ButtonInput
